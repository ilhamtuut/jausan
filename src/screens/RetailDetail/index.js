...
class DetailRetail extends Component {
  constructor(props) {
    super(props);
    this.state = {
      load : false
  ...
  onShare = async () => {
    try {
      ...
      const result = await Share.share({
        message: this.Capitalize(params.nama_perusahaan)
      });
    ...
    return (
      <Container>
        <ImageBackground source={bg} style={Style.backgroundImageLogin}>
          <ThemeHeader
            PageTitle={params.nama_perusahaan}
            IconLeft="arrow-back"
            route="homepage"
          ...
          >
            <Grid style={{ marginBottom: 5, backgroundColor: '#fff' }}>
              <Col style={{ justifyContent: "center", alignItems: 'center' }}>
                <Image style={{ height: 250, width: deviceWidth, alignItems: "center", justifyContent: "center", resizeMode: "contain" }} source={{uri: params.image}} />
              </Col>
            </Grid>
            {/*<TouchableOpacity
              activeOpacity={1}
              ...
              >
              <Text style={{ color: '#f68002', textAlign: 'center', padding: 5  }}><Icon style={{ color: '#f68002', fontSize: 16 }} name="share"/> Bagikan</Text>
            </TouchableOpacity>*/}
            <View style={{ backgroundColor: '#fff', marginBottom: 20 }}>
              <ListItem style={{ marginLeft: 0 }}>
                <Body>
                  <Text style={{ color: '#f68002', fontSize: 14 }}>Nama</Text>
                  <Text note>{this.Capitalize(params.nama_perusahaan)}</Text>
                </Body>
              </ListItem>
              <ListItem style={{ marginLeft: 0 }}>
                <Body>
                  <Text style={{ color: '#f68002', fontSize: 14 }}>Pemilik</Text>
                  <Text note>{this.Capitalize(params.nama_pemilik)}</Text>
                </Body>
              </ListItem>
              <ListItem style={{ marginLeft: 0 }}>
                <Body>
                  <Text style={{ color: '#f68002', fontSize: 14 }}>Email</Text>
                  <Text note>{params.email}</Text>
                </Body>
              </ListItem>
              <ListItem style={{ marginLeft: 0 }}>
                <Body>
                  <Text style={{ color: '#f68002', fontSize: 14 }}>No. Handphone</Text>
                  <Text note>{this.Capitalize(params.no_hp)}</Text>
                </Body>
              </ListItem>
              <ListItem style={{ marginLeft: 0 }}>
                <Body>
                  <Text style={{ color: '#f68002', fontSize: 14 }}>Alamat</Text>
                  <Text note>{this.Capitalize(params.alamat)}</Text>
                </Body>
              </ListItem>
              <ListItem style={{ marginLeft: 0 }}>
                <Body>
                  <Text style={{ color: '#f68002', fontSize: 14 }}>Kecamatan</Text>
                  <Text note>{this.Capitalize(params.kecamatan)}</Text>
                </Body>
              </ListItem>
              <ListItem style={{ marginLeft: 0 }}>
                <Body>
                  <Text style={{ color: '#f68002', fontSize: 14 }}>Kategori</Text>
                  <Text note>{this.Capitalize(params.lapak ? params.lapak.name : '-')}</Text>
                </Body>
              </ListItem>
              <ListItem style={{ marginLeft: 0 }}>
                <Body>
                  <Text style={{ color: '#f68002', fontSize: 14 }}>Website</Text>
                  <Text note onPress={() => Linking.openURL(params.web)}>{params.web}</Text>
                </Body>
              </ListItem>
              <ListItem style={{ marginLeft: 0 }}>
                <Body>
                  <Text style={{ color: '#f68002', fontSize: 14 }}>Akun Media Sosial</Text>
...
export default connect(mapStateToProps, bindAction)(DetailRetail);
